...
加速比为原版总时长/方法总时长。原版为本轮新生成，对每档使用相同prompt、seed与全部生成配置。每条指标比较所有121帧；SSIM不是VBench官方评分。四条提示词与速度映射实验的独立验证提示词相同，因此可以横向查看两组，但不同轮次的计时仍分别使用各自原版归一化。
...
图上我们的方法使用本次直接beta五档，其它方法使用v3已验证的历史轮次。同一提示词、种子、配置和GPU；本轮原版与历史原版视频逐位一致，各自以对应轮次的原版计时归一化。图不是全方法同轮测试，也不是上表4条验证均值。WAN速度映射的两个阈值另见[映射报告](WAN_SPEED_RESULTS.md)。
...
本固定beta实验32条生成：1条排除的预热，4×6条验证，1×6条图表样本，1条末尾原版回放；共25对质量评分。恢复器未重新训练，原11档与唯一ProfilingDiT适配不变。
...
统一包包含原11档、速度映射2档、直接beta5档，共18档。默认完整VBench18×6,220=111,960条，本次没有运行；也可通过`--methods`明确选择其中一组。恢复器训练涉及VBench提示词，不称完整VBench为全未见。
