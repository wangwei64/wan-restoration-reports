...
本补充版共18个可运行配置：v3原11档、2个WAN速度映射档、5个直接LTX beta固定值。新增测试见docs/WAN_SPEED_RESULTS.md与docs/DIRECT_BETA_RESULTS.md。
...
默认覆盖官方 946 条元数据记录，完全重复的文本合并为 944 条，保留原始记录编号和所有维度。普通提示词生成 5 个种子；75 条 temporal_flickering 提示词生成 25 个种子供官方静态筛选。每个方法 6,220 条视频，18 个方法/档位共 111,960 条。种子根据基础 seed、提示词和重复编号确定，与方法、执行顺序及机器分片无关。
...
本发布包只提供一个 ProfilingDiT 版本：`profiling_official`。原有其它10个原版/方法档位保持原配置，另增加两个速度映射档。验证/测试提示词与旧实验相同，不能声称首次未见；我们的方法训练提示词覆盖VBench。

## WAN beta 3 / 4 速度映射扩展

本补充版保留 v3 的 11 个配置及其推理代码，另加两个只修改 LTX controller.budget 的配置。此前三个档按质量选择，这两个新档按相对原版的在线加速比选择，不能称为质量等价档。所有流速、帧数、步数、分辨率和 CFG/STG 参数不变。

| 新入口 | LTX beta | WAN 目标加速比 | 4条独立验证实测 |
|---|---:|---:|---:|
| `ours_wan_speed_3` | 2.969 | 2.715x | 2.796x |
| `ours_wan_speed_4` | 3.809 | 2.810x | 2.902x |

## 直接使用LTX beta的五档

这五档固定LTX beta=0.1、1.0、2.0、3.0、4.0，不做WAN映射，不根据验证结果选参。入口如下：

| 入口 | LTX beta | 4条验证加速比 | SSIM | LPIPS |
|---|---:|---:|---:|---:|
| `ours_beta_0p1` | 0.1 | 1.4900x | 0.98945 | 0.00334 |
| `ours_beta_1p0` | 1.0 | 2.2402x | 0.97529 | 0.01903 |
| `ours_beta_2p0` | 2.0 | 2.6147x | 0.96535 | 0.02642 |
| `ours_beta_3p0` | 3.0 | 2.8026x | 0.95494 | 0.03310 |
| `ours_beta_4p0` | 4.0 | 2.9190x | 0.95092 | 0.03956 |

全部18档完整采样共18×6,220=111,960条，完整VBench仍未在本机完成。`--methods` 可显式选取需要的入口。恢复器权重完全不变，继续通过原v3公开Release的固定哈希下载，无需重新训练。

本轮测速质量见 [WAN速度映射报告](docs/WAN_SPEED_RESULTS.md)和[直接beta五档](docs/DIRECT_BETA_RESULTS.md)。旧v3证据在tests/evidence_v3，其11档计数与原配置哈希属于历史版本，不表示本次18档已经重新进行全量同轮测试。
