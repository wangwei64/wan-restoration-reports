"""LTX-Video 128-channel adaptation of the user-owned Wan restoration modules.

The architecture and matched hidden weights are reused; input/output projections
and confidence calibration require LTX-Video teacher training.
"""
...
class RestorationBackbone(nn.Module):
    def __init__(self, config):
        ...
        c = self.config
        self.intro = nn.Conv3d(128, c.width, (1, 3, 3), padding=(0, 1, 1))
        self.ending = nn.Conv3d(c.width, 128, (1, 3, 3), padding=(0, 1, 1))
        ...
        self.detail_latent = nn.Conv3d(896, dw, 1)
        ...
        self.detail_ending = nn.Conv3d(dw, 128, (1, 3, 3), padding=(0, 1, 1))
    ...
    def forward(self, z10, heat):
        ...
        pad_mode = "reflect" if ph < height and pw < width else "replicate"
        z = F.pad(z10, (0, pw, 0, ph, 0, 0), mode=pad_mode) if ph or pw else z10
...
class FeatureRestorer(nn.Module):
    ...
    def forward(self, z10, features, heat):
        ...
        expected = 0.10 * F.softplus(q[:, :1])
        cfg = self.config["adapter"]
        risk = torch.sigmoid(
            (expected - cfg["latent_error_tolerance"]) / cfg["latent_error_softness"]
        )
        return (
            restored,
            1 - risk,
            {("expected_rgb_error" if cfg.get("quality_error_space")=="rgb" else "expected_latent_error"): expected, "latent_risk": torch.sigmoid(q[:, 1:]),
             "latent_risk_logits": q[:, 1:]},
        )
...
class LatentFeatures(nn.Module):
    def __init__(self, width=48, blocks=4):
        super().__init__()
        self.condition = nn.Sequential(nn.Linear(1, width), nn.SiLU(), nn.Linear(width, width))
        self.net = nn.Sequential(
            nn.Conv3d(128, width, 3, padding=1),
            *[TemporalNAFBlock(width) for _ in range(blocks)],
            LayerNorm3d(width),
            nn.Conv3d(width, 47, 1),
        )

    def forward(self, z, sigma=0.88):
        x = self.net[0](z)
        condition = torch.as_tensor(sigma, device=z.device, dtype=torch.float32).reshape(-1, 1)
        if len(condition) == 1:
            condition = condition.expand(z.shape[0], -1)
        x = x + self.condition(condition)[:, :, None, None, None]
