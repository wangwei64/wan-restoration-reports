...
- ProfilingDiT：GeekGuru123/ProfilingDiT，提交 `fa9d1983418b481cc37eb69e7f6c5a3ec6e182a1`。accelerators/vendor/profilingdit/cache.py与该提交原文件字节一致，在线直接调用DitCache.forward_single_original；本地仅提供LTX调用与清理包装，参数采用公开默认值及明确的层比例映射。LTX 离线前景来自 Grounding DINO + SAM2 的 RGB 分割，区别于原论文前景处理；不是作者官方 LTX 代码。固定层列表的在线使用不需要分割模型。
